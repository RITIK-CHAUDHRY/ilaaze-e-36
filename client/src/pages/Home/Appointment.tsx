...
const AppointmentPage = () => {
  ...
  return (
    <div className="min-h-screen bg-teal-50 py-10 px-6">
        <Navbar/>
      <h1 className="text-4xl font-bold text-center text-teal-700 mb-10">Available Doctors</h1>
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {doctors.map((doctor) => (
          <div
            key={doctor.id}
            className="bg-white p-6 rounded-2xl shadow-lg hover:shadow-2xl transition duration-300 border-t-4 border-teal-500"
          >
            ...
            <p className="text-gray-600">Available: <span className="font-medium">{doctor.availableTime}</span></p>
            <button
              onClick={() => handleAppointment(doctor)}
              className="mt-4 bg-teal-600 hover:bg-teal-700 text-white px-4 py-2 rounded-xl shadow"
            >
              Start Appointment
            </button>
          </div>
